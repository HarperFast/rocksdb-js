...
- Old log files can be automatically purged based on retention policy. The highest sequence file is
  retained as the live store's durable floor, so an idle store can keep one bounded file past the
  cutoff until a later write rotates it.
