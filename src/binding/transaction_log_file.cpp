...
ssize_t TransactionLogFile::readFromFile(void* buffer, size_t size, off_t offset) {
#ifdef PLATFORM_WINDOWS
	if (!FILE_IS_VALID(this->fileHandle)) {
		this->open();
	}
...
ssize_t TransactionLogFile::writeToFile(const void* buffer, size_t size, off_t offset) {
#ifdef PLATFORM_WINDOWS
	if (!FILE_IS_VALID(this->fileHandle)) {
		this->open();
	}
