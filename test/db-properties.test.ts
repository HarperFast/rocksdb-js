...
describe('Database Properties', () => {
	it('should get string property from database', () =>
		dbRunner(async ({ db }) => {
	...
	it('should get num-files-at-level property', () =>
		dbRunner(async ({ db }) => {
	...
	it('should test blob files with num-blob-files property', () =>
		dbRunner(async ({ db }) => {
	...
	it('should get live-blob-file-size property', () =>
		dbRunner(async ({ db }) => {
			// Create large values
	...
	it('should get total-blob-file-size property', () =>
		dbRunner(async ({ db }) => {
			// Create large values
			const largeValue = 'x'.repeat(10000);
			await db.put('blob1', largeValue);
	...
	it('should get background-errors property', () =>
		dbRunner(async ({ db }) => {
	...
	it('should get cur-size-all-mem-tables property', () =>
		dbRunner(async ({ db }) => {
			await db.put('key1', 'value1');
			await db.put('key2', 'value2');
