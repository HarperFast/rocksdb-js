...
TEST(TransactionLogClockFloor, TheWritePathAppliesTheFloorsOwnRule) {
	auto storePath = uniqueStorePath();
	std::filesystem::create_directories(storePath);
	auto store = loadStore(storePath);
	ASSERT_NE(store, nullptr);
...
TEST(TransactionLogClockFloor, AFarHeaderWalksEveryOlderSegment) {
	auto storePath = uniqueStorePath();
	std::filesystem::create_directories(storePath);
	const double high = futureKey();
	const double far = rocksdb_js::MAX_TIMESTAMP_MS - 1.0;
...
TEST(TransactionLogClockFloor, APlausibleHeaderNewerThanTheFarOneIsNotTrusted) {
	auto storePath = uniqueStorePath();
	std::filesystem::create_directories(storePath);
	const double high = futureKey();
	const double far = rocksdb_js::MAX_TIMESTAMP_MS - 1.0;
