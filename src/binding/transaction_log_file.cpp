...
void TransactionLogFile::open() {
	...
	if (this->size == 0) {
		...
		result = this->readFromFile(buffer, 2, 4);
		if (result < 0) {
			DEBUG_LOG("%p TransactionLogFile::open ERROR: Failed to read version from file: %s\n", this, this->path.string().c_str())
			throw std::runtime_error("Failed to read version from file: " + this->path.string());
		}
...
void TransactionLogFile::writeEntriesV1(TransactionLogEntryBatch& batch, const uint32_t maxFileSize) {
	...
	// 	DEBUG_LOG("%p TransactionLogFile::writeEntries No entries to write (availableSpaceInFile=%lli)\n", this, availableSpaceInFile)
	// 	return;
	// }
	...
	// DEBUG_LOG("%p TransactionLogFile::writeEntries Will write %u entries (totalTxnSize=%u, currentBlockSize=%u, availableSpaceInCurrentBlock=%u, dataForCurrentBlock=%u, numNewBlocks=%u, dataForNewBlocks=%u, availableSpaceInFile=%lli, initialDataOffset=%u)\n",
	...
	// 			DEBUG_LOG("%p TransactionLogFile::writeEntries ERROR: iovec overflow! index=%zu, max=%u\n",
	// 				this, iovecsIndex, maxIovecs)
	// 			throw std::runtime_error("iovec array overflow in writeEntries");
	// 		}
	// 		iovecs[iovecsIndex++] = {base, len};
	// 		DEBUG_LOG("%p TransactionLogFile::writeEntries Added iovec[%zu]: base=%p, len=%zu\n",
	...
	// 		DEBUG_LOG("%p TransactionLogFile::writeEntries ERROR: block header overflow! index=%u, max=%u\n",
	// 			this, nextBlockHeaderIdx, maxBlockHeaders)
	// 		throw std::runtime_error("block header overflow in writeEntries");
	// 	}
	...
	// 	DEBUG_LOG("%p TransactionLogFile::writeEntries Block %u header: timestamp=%f, flags=%u, dataOffset=%u\n",
	...
	// 			DEBUG_LOG("%p TransactionLogFile::writeEntries Added %u bytes of padding to block %u\n",
	...
	// 				DEBUG_LOG("%p TransactionLogFile::writeEntries Updated block %u dataOffset to %u (excluding %u bytes of padding)\n",
	...
	// 		DEBUG_LOG("%p TransactionLogFile::writeEntries Advanced to block %u (continuation=%d)\n", this, currentBlockIdx, isContinuation)
	// 	}
	// };
	...
	// 	char* blockHeader = createBlockHeader(isContinuation, dataOffset);
	// 	addIovec(blockHeader, BLOCK_HEADER_SIZE);
	// 	DEBUG_LOG("%p TransactionLogFile::writeEntries Added initial block header 0 (continuation=%d)\n", this, isContinuation)
	// }
	...
	// 	// validate entry
	// 	if (!entry) {
	// 		DEBUG_LOG("%p TransactionLogFile::writeEntries ERROR: null entry at index %zu\n", this, entryIdx)
	// 		throw std::runtime_error("null entry in writeEntries");
	// 	}
	// 	if (entry->data == nullptr) {
	// 		DEBUG_LOG("%p TransactionLogFile::writeEntries ERROR: null entry data at index %zu\n", this, entryIdx)
	// 		throw std::runtime_error("null entry data in writeEntries");
	// 	}
	...
	// 	DEBUG_LOG("%p TransactionLogFile::writeEntries Processing entry %zu (size=%zu, startOffset=%u, remainingSize=%u, data=%p)\n",
	...
	// 			DEBUG_LOG("%p TransactionLogFile::writeEntries ERROR: txnHeaderIdx overflow! index=%u, max=%u\n",
	// 				this, txnHeaderIdx, numEntriesToWrite)
	// 			throw std::runtime_error("txnHeader index overflow in writeEntries");
	// 		}
	...
	// 			DEBUG_LOG("%p TransactionLogFile::writeEntries Transaction header won't fit (available=%u, needed=%u), advancing to next block\n",
	...
	// DEBUG_LOG("%p TransactionLogFile::writeEntries Built %zu iovecs for writing\n", this, iovecsIndex)
	...
	// DEBUG_LOG("%p TransactionLogFile::writeEntries Updated currentBlockSize=%u, added %u blocks, total blockCount=%u, batch state: entryIndex=%zu, bytesWritten=%zu\n",
...
// 	if (maxIndexFileSize > 0) {
// 		if (this->indexFileSize >= maxIndexFileSize) {
...
// 				this, this->indexFileSize, maxIndexFileSize)
// 			return 0;
// 		}
// 		availableSpace = maxIndexFileSize - this->indexFileSize;
// 	}
