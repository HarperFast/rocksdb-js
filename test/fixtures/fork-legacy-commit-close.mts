...
if (!dbPath) {
	throw new Error('Usage: fork-legacy-commit-close.mts <dbPath> [commit|probe]');
}
...
assert(
	!registryStatus().some((entry) => entry.path === dbPath),
	'last-handle close left the descriptor in the registry after commit completion'
);
