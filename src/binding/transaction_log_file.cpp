...
ssize_t TransactionLogFile::readData(void* buffer, size_t size, off_t offset) {
#ifdef PLATFORM_WINDOWS
	if (!FILE_IS_VALID(this->fileHandle)) {
		this->open();
	}
...
ssize_t TransactionLogFile::writeData(const void* buffer, size_t size, off_t offset) {
#ifdef PLATFORM_WINDOWS
	if (!FILE_IS_VALID(this->fileHandle)) {
		this->open();
	}
