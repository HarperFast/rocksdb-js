...
export async function buildRocksDBFromSource(rocksdbPath: string, dest: string): Promise<void> {
	...
	execFileSync('make', ['-C', rocksdbPath, 'static_lib'], { stdio: 'inherit' });
	...
	await symlink(resolve(rocksdbPath, 'include'), resolve(dest, 'include'));

	await symlink(resolve(rocksdbPath, 'librocksdb.a'), resolve(dest, 'lib/librocksdb.a'));
}
