 ...
 * to the consumer.
 *
 * Only what a RocksDB backup needs is implemented: regular files with short,
 * flat names. RocksDB filenames (`000123.sst`, `MANIFEST-000007`, `CURRENT`)
 * fit comfortably in USTAR's 100-byte name field, so the `prefix` field and
 * GNU/pax long-name extensions are intentionally omitted (a name that does not
 * fit throws rather than being silently truncated). File sizes larger than the
 * octal `size` field can hold (~8 GiB) fall back to the GNU base-256 encoding.
 */

/** Size of a tar block. Every header and the archive length are multiples of this. */
const BLOCK_SIZE = 512;

/** Maximum bytes for a USTAR `name` field (no `prefix` splitting). */
...
function buildHeader(name: string, size: number, mode: number, mtime: number): Uint8Array {
	...
	writeField(block, name, 0, MAX_NAME_LENGTH);
	...
	writeOctal(block, mtime, 136, 12); // mtime
...
export class TarEncoder {
	...
	async addFile(name: string, size: number, options?: TarEntryOptions): Promise<void> {
		...
		if (textEncoder.encode(name).length > MAX_NAME_LENGTH) {
			throw new RangeError(
				`tar entry name ${JSON.stringify(name)} exceeds ${MAX_NAME_LENGTH} bytes`
			);
		}
		this.#remaining = size;
		this.#pad = (BLOCK_SIZE - (size % BLOCK_SIZE)) % BLOCK_SIZE;
		await this.#sink(buildHeader(name, size, options?.mode ?? 0o644, options?.mtime ?? 0));
	}
