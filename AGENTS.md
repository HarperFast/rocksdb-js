...
20. **A secondary open's identity is `{path, readOnly, secondaryPath}` and its workspace is
...
21. **A transaction timestamp freezes when native state captures it**: `setTimestamp()` may adopt an
