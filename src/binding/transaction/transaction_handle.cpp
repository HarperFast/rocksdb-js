...
void TransactionHandle::close() {
	...
	// Test seam: widen the PATH A vs PATH B race window.
	...
	// Noop in production.
	const int closeDelayMs = txnCloseDelayMsFlag().load(std::memory_order_relaxed);
	if (closeDelayMs > 0) {
...
napi_value TransactionHandle::get(
	...
	const int getDelayMs = txnGetDelayMsFlag().load(std::memory_order_relaxed);
			if (getDelayMs > 0) {
