...
11. **A corrupt transaction-log frame ends an entry, not the log**: framing breaks come in two
    ...
    `readableExtent` described in invariant 30, seeded from append-owned
...
31. **`databaseFlushed()` persists and verifies `txn.state` by pathname**: a stream kept open across
