...
/**
 * Opens the `.backup.lock` file inside `backupDir` (creating it if missing) and
 * takes a non-blocking exclusive advisory lock on it — `flock` on POSIX,
 * `LockFileEx` on Windows. Returns an opaque non-zero token to pass to
 * `releaseFileLock`, `0` if another holder currently has the lock, or throws
 * `DBException` on a hard error (including a clear "does not exist" message when
 * `backupDir` is missing).
 *
