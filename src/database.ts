...
export class RocksDatabase extends DBI<DBITransactional> {
	 ...
	 * `'database:closeFailed'` with `(path, error)` string arguments. A failure
	 * that prevents teardown quarantines the path until `destroy()` or
	 * `shutdown()` retries cleanup.
	 *
	 * @example
	 * ```typescript
	...
	async transaction<T>(
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			...
			try {
				...
				if (
					commitErr instanceof TransactionRetryableError &&
	...
	transactionSync<T>(
		...
		const isRetryable = (err: Error | unknown, attempt: number) => {
			return (
				err instanceof TransactionRetryableError &&
