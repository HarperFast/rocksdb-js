...
11. **A corrupt transaction-log frame ends an entry, not the log**: framing breaks come in two
    ...
    `readableExtent` described in invariant 29, seeded from append-owned
    ...
    Resolve it only on a break: even the mapping-carried getter crosses into native, so a per-frame
    call would tax every healthy read. The native walks run before `size` has been
...
30. **A purged transaction-log segment stays readable through a mapping that already exists, but
...
30. **`databaseFlushed()` persists and verifies `txn.state` by pathname**: a stream kept open across
