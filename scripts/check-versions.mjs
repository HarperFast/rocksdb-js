...
if (releaseTag) {
	...
	if (tagVersion !== version) {
		console.error(
			`ERROR: Release tag version "${tagVersion}" does not match package.json version "${version}"!`
		);
		process.exit(1);
	}
}
...
const mismatched = Object.entries(optionalDependencies).filter(([_key, value]) =>
	value !== version
);
