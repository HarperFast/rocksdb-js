...
| B10 | Overlapping long transactions + forced reserve rollover under load                                                                         | **deferred as a latency gate** (recorded openly); the structural half is enforced in code — reserve I/O cannot run under a log `writeMutex` (`ensureHeadroom` precedes the lock; the under-lock claim never extends) — and F8 crash-tests the extension boundary                                                                   |
