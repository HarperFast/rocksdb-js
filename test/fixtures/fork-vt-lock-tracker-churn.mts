 ...
 * shape: racer / stager / abandoner). One worker is recycled periodically via
 * a GRACEFUL stop (finish current iteration, close its own DB handle, exit)
 * while the other workers are still mid-commit through the same
 * process-global DBDescriptor (AGENTS.md note 6).
 *
...
const recycleEveryMs = Number(process.argv[5] ?? 1_500);
...
function stopWorkerGracefully(id: number): Promise<void> {
	...
	return new Promise<void>((resolve) => {
		worker.once('message', (msg: { stopped?: boolean }) => {
			if (msg.stopped) resolve();
		});
